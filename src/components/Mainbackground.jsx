...
const CircleAnimation = () => {
  ...
  return (
    <div className="absolute right-1 bottom-1 md:right-2 md:bottom-2  w-[200px] h-[200px] rounded-full flex justify-center items-center ring-0 ring-slate-100 shadow-[0_0_10px_#fefefe]  overflow-hidden">
      <div
        style={{ background: `linear-gradient(to top,rgba(0,0,0,0.9),transparent 60%),url(${logo})no-repeat center/70% 100%` }}
        className="absolute inset-0 w-full brightness-110 contrast-[1.1]"
      ></div>
      <div ref={textRef} className="circle-text">
        @Saran~❤️Web Developer~❤️Youtuber~❤️
      </div>
    </div>
  );
};
